...
class Decoder(nn.Module):
    def __init__(self, num_abs_agents,num_agents,abs_action_space_dimension, action_dim):
        super(Decoder, self).__init__()
        # deprecated decode map
        # self.decode_map=torch.randint(low=0,high=action_dim, size=(num_abs_agents, abs_action_space_dim, agents_per_abs_agent))
        ...
        if index_mode:
            ...
            self.partition= ? # a differentiable state-to-partition-element function (partition element represented as onehot) that partitions the embedding space into num_abs_agents regions (e.g. if using hyperplanes, would need log2(num_abs_agents) of them)
    ...
    def init_index_mode_partition(num_abs_agents,num_agents, init_prob=0.99):
        ...
        partition=np.zeros((num_agents,num_abs_agents))
        partition[abs_agent_idxs] = init_prob
        partition[~abs_agent_idxs] = partition[abs_agent_idxs]
    ...
    def decode(self, abs_actions):
        ...
        for agent_idx in range(num_agents):
            ...
            abs_agent_idx, abs_action = get_abs_agent_and_action(abs_actions,agent_idx, self.partition)

            # put that through an (agent specific?) network that gives the action
            action = policy(abs_agent_idx, abs_action)
            output_actions.append(action)

        # deprecated decode_map
    ...
    def get_abs_agent_and_action(abs_actions,agent_idx, partition):
        assignment_probabilities = get_abs_agent_assignment_probabilities(agent_idx)
        logits = torch.log(assignment_probabilities.shape/(1-assignment_probabilities.shape))
