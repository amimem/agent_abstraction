...
class GroundModelJointPolicy(nn.Module):
    def __init__(self, state_space_dim, num_abs_agents, agents_per_abstract_agent, action_space_dim=2, baseline = 'bitpop', baseline_paras=None):
        ...
        self.state_set= np.array([np.array(l) for l in list(map(list, itertools.product([0, 1], repeat=state_space_dim)))]) #all vertices of unit hypercube
        self.num_states = len(self.state_set)
        self.action_space_dim = action_space_dim

        rng = np.random.default_rng()
        self.action_policies=np.zeros((self.num_agents,self.num_states), dtype=bool)
        ...
        if baseline == 'bitpop':
            ...
            for abs_agent_idx in range(num_abs_agents):
                agent_indices = range(abs_agent_idx * agents_per_abstract_agent, (abs_agent_idx + 1) * agents_per_abstract_agent)
                ...
                if (gen_type == "mix"):  # Bernoulli mixture of independent and identical binary RVs
                    ...
                    self.action_policies[np.ix_(agent_indices_bool, ~is_same)] = rng.integers(0, 2, [agents_per_abstract_agent, n_diff])
